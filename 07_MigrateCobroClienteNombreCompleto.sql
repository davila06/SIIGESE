-- =====================================================
-- SCRIPT PARA MIGRACIÓN DE CAMPOS DE CLIENTE EN TABLA COBROS
-- =====================================================
-- Fecha: 2025-12-16
...
PRINT '🔄 Iniciando migración de campos de cliente en Cobros...';
...
IF NOT EXISTS (
    ...
    WHERE object_id = OBJECT_ID('Cobros') 
...
IF NOT EXISTS (SELECT 1 FROM [Cobros] WHERE [ClienteNombreCompleto] IS NULL)
BEGIN
    -- Eliminar índices que usen estas columnas (si existen)
    IF EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IX_Cobros_ClienteNombre')
        DROP INDEX [IX_Cobros_ClienteNombre] ON [Cobros];
    
    IF EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IX_Cobros_ClienteApellido')
        DROP INDEX [IX_Cobros_ClienteApellido] ON [Cobros];
    
    -- Eliminar columnas
    ALTER TABLE [Cobros] DROP COLUMN [ClienteNombre];
    ALTER TABLE [Cobros] DROP COLUMN [ClienteApellido];
...
IF NOT EXISTS (
    ...
    WHERE name = 'IX_Cobros_ClienteNombreCompleto' 
    AND object_id = OBJECT_ID('Cobros')
)
BEGIN
    PRINT '📑 Creando índice para ClienteNombreCompleto...';
    
    CREATE NONCLUSTERED INDEX [IX_Cobros_ClienteNombreCompleto]
    ON [Cobros] ([ClienteNombreCompleto])
    INCLUDE ([NumeroRecibo], [NumeroPoliza], [Estado]);
    
    PRINT '✅ Índice creado';
END
ELSE
BEGIN
    PRINT '⚠️  Índice IX_Cobros_ClienteNombreCompleto ya existe';
END
GO
...
SELECT 
    ...
    CASE 
        WHEN EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID('Cobros') AND name = 'ClienteNombre')
        THEN 0
        ELSE 1
    END;
...
PRINT '    Descoméntalas sección 5 del script cuando el backend y frontend estén actualizados';
PRINT '';
